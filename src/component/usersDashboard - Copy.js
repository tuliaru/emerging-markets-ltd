...
const UserDashboard = () => {
    ...
    const getListsOfLatestRates = ( pg,search="",region="",q )=> {
        setLoading(true);
       
        axiosURL.get('/get-latest-rates',{headers:{
        'Authorization':'Bearer ' + userToken?.token
        },params:{page: pg,search_date:search,region,q}}).then(res => {
        console.log(res);
    ...
    const handlePagination = (pageNo,filterDate,regionfilter,select) => {
        ...
        if(select !== ''){
            getListsOfLatestRates(pageNo,filterDate,'',select);
        }
        else{
            getListsOfLatestRates(pageNo,filterDate,regionfilter,'');
        }
    }
    ...
    <>
      ...
      {/*row heading start*/}
        ...
        <div className="light-body-bg shadow-lg py-4 px-4 rounded-4">
            ...
            <div className="dataTables_length"><h5 className="text-info mb-0 d-inline-block">{totalRecords} results found</h5>
        ...
        <DatePicker
                ...
                />
                <button className="btn btn-light rounded-0 top-0 end-0 z-4 btn-sm px-2 py-1" type="button" id="button-addon2" onClick={()=>handleSearch(searchDate)}>GO</button>
             </div>
            </div>
            </div>
            </div>
            <div><h5 className="text-info">The exchange rates displayed below indicate sell rates of USD.</h5></div>
            ...
            <nav aria-label="Page navigation example">
                <ul className="pagination pagination-sm justify-content-end mt-4">
                             ...
                             {[...Array(pagination)].map((x, i) =>
                                <li className="page-item" key={i} onClick={()=>handlePagination(i+1,searchDate,regionfilter,select)}>
