      ...
      {/*row heading start*/}
        ...
        <div className="light-body-bg shadow-lg py-4 px-4 rounded-4">
            ...
            <div className="dropdown ms-3">
              ...
              <button className="btn btn-secondary btn-sm dropdown-toggle" id="dropdownRateUploadButton" data-bs-toggle="dropdown" onClick={(e)=>{
                      let rateUploadDropdownLinks = new Dropdown(e.target);
                      rateUploadDropdownLinks.show();
              }}>
                Upload Rate
              </button>
				...
				listLatestRates.map((item,i) => {
					...
					<tr role="row" className="odd" key={i} data-rowid={item?.row_id}>
						...
						<td className="align-middle" key={Math.random()}>
                            ...
                            <button type="button" className="btn btn-sm btn-light btn-update-localrate rounded-2"  onClick={(e) => handleUpdateLocalRate(e.currentTarget,item?.currency_id,e.currentTarget.parentNode.querySelector(".input-local-rate").value)} style={{display:"none"}}><i className="fa-solid fa-pen-to-square"></i></button>
                           <button type="button" className="btn btn-sm btn-light btn-cancel-localrate rounded-2 bg-red border-red ms-2" style={{display:"none"}} onClick={(e) => handleClickCancelLocalRate(e.currentTarget,item?.local_market_rate)}><i className="fa-regular fa-circle-xmark fa-lg"></i></button>
                        </td>
						<td className="align-middle" key={Math.random()}>
                            <span className="text-citybank-rate">{item?.city_bank_rate}</span>
                            <img className="align-middle ms-2 img-edit-citybank-rate" src={IMAGES.EditIcon} alt="edit" onClick={(e)=>handleClickEditCitybankRate(e.currentTarget,item?.city_bank_rate)}/>
                            <input type="text" className="w-25 input-citybankrate-rate" style={{display:"none"}} />
                            <button type="button" className="btn btn-sm btn-light btn-update-citybankrate rounded-2"  onClick={(e) => handleUpdateCitybankRate(e.currentTarget,item?.currency_id,e.currentTarget.parentNode.querySelector(".input-citybankrate-rate").value)} style={{display:"none"}}><i className="fa-solid fa-pen-to-square"></i></button>
                            <button type="button" className="btn btn-sm btn-light btn-cancel-citybankrate rounded-2 bg-red border-red ms-2" style={{display:"none"}} onClick={(e) => handleClickCancelCitybankRate(e.currentTarget,item?.city_bank_rate)}><i className="fa-regular fa-circle-xmark fa-lg"></i></button>
                        </td>
