...
const UserDashboard = () => {
    ...
    const handleInterval = () => {
        if(intervalRef1.current>0)
        {
            clearInterval(intervalRef1.current);
            intervalRef1.current = 0;
        }
        intervalRef1.current = setInterval(() => {
                
            let month = searchDateRef.current.getMonth() + 1;
            let day = searchDateRef.current.getDate();
            let year = searchDateRef.current.getFullYear();
            ...
            getListsOfLatestRates(1,sDate,regionRef.current,select);
            handleInterval();
        }, 120000);
    ...
    useEffect(() => {
        ...
        return () => {
          clearInterval(intervalRef1.current);
          intervalRef1.current = 0;
          searchDateRef.current = new Date();
          regionRef.current = 0;
    ...
    const handleDateChange = (dt) => {
        ...
        searchDateRef.current = new Date(setChangedDate);

        getListsOfLatestRates(1,setChangedDate,regionRef.current,select)
